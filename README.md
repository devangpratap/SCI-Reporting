...
Built with React + Vite (frontend), Express (API), Databricks SQL (data source), Cytoscape.js (dependency graph), and an MCP server so Claude can query the data directly in natural language.
...
| Dependency Graph | Cytoscape.js + cytoscape-dagre | Canvas-based DAG rendering, handles large graphs efficiently |
...
│           ├── P9Panel.jsx   Stalls + Dependency Graph (Cytoscape.js)
...
## P9 — Dependency Graph (Cytoscape.js)

### How it works

The P9 panel renders a directed acyclic graph of task dependencies with the critical path highlighted. Here's the full data flow:

```
Databricks (sci_p9_nodes + sci_p9_edges + sci_p9_stalls)
    ↓
server/db.js → getGraph()
    ↓ BFS/DFS longest-path algorithm
    ↓ annotates each node with on_critical_path: bool, is_stalled: bool
    ↓
GET /api/p9/graph → { nodes[], edges[] }
    ↓  (direct fetch, NOT through MCP — graph is static, no token cost)
P9Panel.jsx → Cytoscape.js canvas
```

**Critical path computation runs server-side** in `db.js:getGraph()`. The algorithm:
1. Builds an adjacency list from edges
2. Runs DFS to compute longest-path distance from each node
3. Traces the critical path back from the root with highest distance
4. Annotates every node with `on_critical_path: true/false` and `is_stalled: true/false`

The frontend receives pre-computed flags and applies visual styles — no graph algorithms in the browser.

### Visual legend

| Color | Meaning |
|---|---|
| Cyan border | Normal node |
| Orange border + dark brown fill | On critical path |
| Red dashed border | Stalled (blocked, no progress) |
| Green border | Completed |
| Orange edge | Critical path dependency |
| Blue edge | Non-critical dependency |

### Interacting with the graph

- **Scroll** to zoom in/out
- **Click + drag** to pan
- **Dagre LR layout** — nodes flow left-to-right by dependency rank

### Cytoscape.js gotchas (for future devs)

**1. `"label": "data(label)"` is required**

Without this style property, Cytoscape renders nodes with no text at all. You must explicitly map the data field to the display label:

```js
{
  selector: "node",
  style: {
    "label": "data(label)",   // ← required, or nodes render blank
    "color": "#e2e8f0",
    // ...
  }
}
...
**2. `cytoscape.use(dagre)` must be guarded against double-registration**

Vite HMR re-runs the module on every save. If `cytoscape.use(dagre)` runs twice it throws. Wrap it:

```js
try { cytoscape.use(dagre); } catch (_) { /* already registered on HMR reload */ }
```

**3. Use `useLayoutEffect`, not `useEffect`**

`useLayoutEffect` fires synchronously after DOM paint, guaranteeing the container `div` has real pixel dimensions before Cytoscape touches it. `useEffect` fires asynchronously — the container may have `width: 0` when Cytoscape runs, producing a blank canvas.

**4. Defer `cy.fit()` to `requestAnimationFrame`**

React 18+ Strict Mode double-invokes effects in development, calling the cleanup function (which destroys the Cytoscape instance) synchronously. Moving `cy.fit()` into a `requestAnimationFrame` callback lets the canvas paint before any cleanup can destroy it:
...
**5. Fixed node dimensions required**

Cytoscape 3.x deprecated `width: "label"` and `height: "label"`. Use fixed numeric values:
...
**6. No `box-shadow` in Cytoscape styles**

`box-shadow` is a CSS property, not a valid Cytoscape style property. Use `overlay-color` + `overlay-opacity` for glow effects:
...
**Note:** The P9 dependency graph is intentionally NOT exposed via MCP. The graph is static (fetched once, directly from `/api/p9/graph`), and routing it through MCP would cost tokens on every query with no benefit.
...
- *"Which teams are blocked by undelivered decisions?"*
...
### P9 — Stalls & Graph
Two sections: stall alert cards (severity-badged, with stall type and affected teams), and a Cytoscape.js dependency graph showing task nodes color-coded by role on the critical path. The graph uses dagre LR layout (left-to-right flow by dependency rank). Zoom and pan are enabled. The critical path is computed server-side — frontend just reads pre-computed flags.
...
**To modify the dependency graph:** read the Cytoscape.js gotchas section above before touching `P9Panel.jsx` — there are several non-obvious requirements for the graph to render correctly.
